...
fromTetromino : Tetromino -> Board
fromTetromino tetromino = 
    let toTuple block location = (location, block)
    in  new (List.map (toTuple tetromino.block) tetromino.shape)
    
    
tetromino = Tetromino.j
tetromino' = Tetromino.shift (round tetromino.pivot.c, round tetromino.pivot.r) tetromino
main = collage 600 600 [toForm (fromTetromino tetromino)]